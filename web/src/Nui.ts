...
async function post(event: string, data = {}): Promise<any> {
  if (!import.meta.env.PROD) {
    console.log(`[Nui] Dev mock called for: ${event}`, mocks[event] ? 'FOUND' : 'NOT FOUND');
    if (!mocks[event]) {
      console.warn(`[Nui] No mock found for event: ${event}`);
      return;
    }

    const result = mocks[event](data);
    console.log(`[Nui] Mock result for ${event}:`, result);
    return result;
  }
...
export const EventListener = () => {
  ...
  window.addEventListener('keydown', e => {
    if (e.key === 'd') {
      Nui.post('nex_rotate_right');
    } else if (e.key === 'a') {
      Nui.post('nex_rotate_left');
    }
  });
